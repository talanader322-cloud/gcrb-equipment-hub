...
AS $function$
  ...
  SELECT
    ...
    CASE
      WHEN public.normalize_code(COALESCE(csp.number, '')) = input.code
        ...
        THEN 0
      WHEN public.normalize_code(COALESCE(csp.number, '')) LIKE '%' || input.code || '%'
        OR public.normalize_code(COALESCE(csp.short_number, '')) LIKE '%' || input.code || '%'
        THEN 1
  ...
  WHERE input.raw IS NOT NULL
    AND (
      public.normalize_code(COALESCE(csp.number, '')) LIKE '%' || input.code || '%'
      OR public.normalize_code(COALESCE(csp.short_number, '')) LIKE '%' || input.code || '%'
      OR public.normalize_code(COALESCE(csp.item_ref, '')) LIKE '%' || input.code || '%'
