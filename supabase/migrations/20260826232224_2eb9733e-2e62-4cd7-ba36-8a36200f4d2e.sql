...
CREATE UNIQUE INDEX IF NOT EXISTS uq_machine_models_manufacturer_normalized ON public.machine_models (manufacturer_id, normalized_model_name) WHERE normalized_model_name IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_parts_manufacturer_normalized ON public.parts (manufacturer_id, normalized_part_number) WHERE normalized_part_number IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_catalogs_oem_model_number_revision ON public.catalogs (manufacturer_id, COALESCE(machine_model_id, '00000000-0000-0000-0000-000000000000'::uuid), normalized_catalog_number, COALESCE(revision, '')) WHERE normalized_catalog_number IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_catalogs_oem_model_title_revision_no_number ON public.catalogs (manufacturer_id, COALESCE(machine_model_id, '00000000-0000-0000-0000-000000000000'::uuid), normalized_title, COALESCE(revision, '')) WHERE normalized_catalog_number IS NULL AND normalized_title IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_catalog_machine_relations_nullable_serial ON public.catalog_machine_relations (catalog_id, machine_model_id, COALESCE(serial_range_id, '00000000-0000-0000-0000-000000000000'::uuid));
CREATE UNIQUE INDEX IF NOT EXISTS uq_part_machine_compatibility_nullable_serial ON public.part_machine_compatibility (part_id, machine_model_id, COALESCE(serial_range_id, '00000000-0000-0000-0000-000000000000'::uuid));

-- Scope import history and temporary online-search cache.
DROP POLICY IF EXISTS "import jobs readable" ON public.import_jobs;
DROP POLICY IF EXISTS "import jobs scoped read" ON public.import_jobs;
CREATE POLICY "import jobs scoped read" ON public.import_jobs FOR SELECT TO authenticated USING (user_id = auth.uid() OR public.can_manage_catalog(auth.uid()));

DROP POLICY IF EXISTS "import items readable" ON public.import_job_items;
DROP POLICY IF EXISTS "import items scoped read" ON public.import_job_items;
CREATE POLICY "import items scoped read" ON public.import_job_items FOR SELECT TO authenticated USING (public.can_manage_catalog(auth.uid()) OR EXISTS (SELECT 1 FROM public.import_jobs j WHERE j.id = import_job_id AND j.user_id = auth.uid()));

DROP POLICY IF EXISTS "temp results writable" ON public.external_search_results;
DROP POLICY IF EXISTS "temp results manager insert" ON public.external_search_results;
CREATE POLICY "temp results manager insert" ON public.external_search_results FOR INSERT TO authenticated WITH CHECK (public.can_manage_catalog(auth.uid()));

-- Server-side serial lookup
CREATE OR REPLACE FUNCTION public.search_serial_model_ids(p_query TEXT, p_manufacturer_id UUID DEFAULT NULL, p_equipment_type_id UUID DEFAULT NULL)
...
AS $$
    ...
    SELECT
      ...
      GREATEST(
        CASE
          ...
          WHEN sr.serial_prefix IS NOT NULL AND i.normalized LIKE public.normalize_code(sr.serial_prefix) || '%' THEN 100
          ELSE 0
        END,
        CASE
  ...
  SELECT c.machine_model_id, MAX(c.rank_value)::INT
  FROM candidates c
...
BEGIN
  ...
  INSERT INTO public.import_jobs (source_id, user_id, import_type, status, total_records)
  VALUES (p_source_id, v_user_id, 'online_result', 'running', 1)
  ...
  BEGIN
    ...
    IF v_equipment_type IS NOT NULL THEN
      ...
      SELECT id INTO v_equipment_type_id FROM public.equipment_types
    ...
    IF v_model_name IS NOT NULL AND v_manufacturer_id IS NOT NULL THEN
      IF p_link_model_id IS NOT NULL THEN
        SELECT id INTO v_model_id FROM public.machine_models
        ...
        END IF;
      ELSE
        SELECT id INTO v_existing_id FROM public.machine_models
        ...
        ELSE
          INSERT INTO public.machine_models (manufacturer_id, equipment_type_id, model_name, description)
          VALUES (v_manufacturer_id, v_equipment_type_id, v_model_name, v_part_description)
      ...
      IF v_serial_display IS NOT NULL THEN
        SELECT id INTO v_serial_range_id FROM public.serial_ranges
        WHERE machine_model_id = v_model_id AND display_value = v_serial_display
        LIMIT 1;
        IF v_serial_range_id IS NULL THEN
          INSERT INTO public.serial_ranges (machine_model_id, serial_from, serial_to, display_value, notes)
          VALUES (v_model_id, v_serial_from, v_serial_to, v_serial_display, 'Imported from an approved external source.')
    ...
    IF v_part_number IS NOT NULL AND v_manufacturer_id IS NOT NULL THEN
      SELECT id INTO v_part_id FROM public.parts
      WHERE manufacturer_id = v_manufacturer_id
      ...
      IF v_part_id IS NULL THEN
        ...
        VALUES (v_manufacturer_id, v_part_number, v_part_description, 'Imported from external reference ' || v_external_reference || '.')
      ...
      IF v_model_id IS NOT NULL THEN
        ...
        VALUES (v_part_id, v_model_id, v_serial_range_id,
    ...
    -- Catalog
    IF v_part_number IS NULL AND v_manufacturer_id IS NOT NULL AND (v_catalog_number IS NOT NULL OR v_catalog_title IS NOT NULL) THEN
      IF v_catalog_number IS NOT NULL THEN
        SELECT id INTO v_catalog_id FROM public.catalogs
        WHERE manufacturer_id = v_manufacturer_id
      ...
      IF v_catalog_id IS NULL THEN
        IF v_catalog_title IS NULL THEN
          RAISE EXCEPTION 'Catalog title is required.';
        END IF;
        INSERT INTO public.catalogs (manufacturer_id, machine_model_id, catalog_number, title, catalog_type, language, revision, serial_from, serial_to, source_id, external_source_reference)
        VALUES (v_manufacturer_id, v_model_id, v_catalog_number, v_catalog_title, v_catalog_type, v_language, v_revision, v_serial_from, v_serial_to, p_source_id, v_external_reference)
        ...
        IF v_model_id IS NOT NULL THEN
          ...
          VALUES (v_catalog_id, v_model_id, v_serial_range_id)
          ON CONFLICT DO NOTHING;
        END IF;
      ELSE
    ...
    INSERT INTO public.import_job_items (import_job_id, external_reference, entity_type, status, local_entity_id)
    VALUES (v_job_id, v_external_reference, CASE WHEN v_part_number IS NOT NULL THEN 'part' ELSE 'catalog' END, 'imported', COALESCE(v_part_id, v_catalog_id, v_model_id));

    UPDATE public.import_jobs SET status = 'completed', imported_records = 1, completed_at = now() WHERE id = v_job_id;
  EXCEPTION
    WHEN OTHERS THEN
      v_error := SQLERRM;
      UPDATE public.import_jobs SET status = 'failed', failed_records = 1, error_log = v_error, completed_at = now() WHERE id = v_job_id;
      INSERT INTO public.import_job_items (import_job_id, external_reference, entity_type, status, error_message)
      VALUES (v_job_id, v_external_reference, 'unknown', 'failed', v_error);
      RETURN jsonb_build_object('ok', false, 'jobId', v_job_id, 'error', v_error);
  END;

  RETURN jsonb_build_object('ok', true, 'jobId', v_job_id, 'manufacturerId', v_manufacturer_id, 'modelId', v_model_id, 'catalogId', v_catalog_id, 'partId', v_part_id, 'created', to_jsonb(v_created), 'linked', to_jsonb(v_linked));
END;
$$;
