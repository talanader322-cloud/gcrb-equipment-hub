...
CREATE UNIQUE INDEX IF NOT EXISTS uq_part_machine_compatibility_nullable_serial
  ON public.part_machine_compatibility (
    part_id,
    machine_model_id,
    COALESCE(serial_range_id, '00000000-0000-0000-0000-000000000000'::uuid)
  );
...
CREATE POLICY "import items scoped read"
  ON public.import_job_items FOR SELECT TO authenticated
  USING (
    public.can_manage_catalog(auth.uid()) OR EXISTS (
...
CREATE POLICY "temp results manager insert"
  ON public.external_search_results FOR INSERT TO authenticated
...
-- Atomic external import. Domain writes rollback together; the job itself stays auditable.
  ...
  BEGIN
    IF v_name IS NOT NULL THEN
      ...
      IF v_manufacturer_id IS NULL THEN
        ...
        VALUES (v_name, v_name, v_slug) RETURNING id INTO v_manufacturer_id;
    ...
    IF v_equipment_type IS NOT NULL THEN
      ...
      WHERE slug = v_slug OR LOWER(name) = LOWER(v_equipment_type) LIMIT 1;
      IF v_equipment_type_id IS NULL THEN
        INSERT INTO public.equipment_types (name, slug)
        VALUES (v_equipment_type, v_slug) RETURNING id INTO v_equipment_type_id;
    ...
    IF v_model_name IS NOT NULL AND v_manufacturer_id IS NOT NULL THEN
      ...
      ELSE
        ...
        WHERE manufacturer_id = v_manufacturer_id
          AND normalized_model_name = public.normalize_code(v_model_name) LIMIT 1;
      ...
      IF v_serial_display IS NOT NULL THEN
        ...
        WHERE machine_model_id = v_model_id AND display_value = v_serial_display LIMIT 1;
    ...
    IF v_part_number IS NOT NULL AND v_manufacturer_id IS NOT NULL THEN
      ...
      WHERE manufacturer_id = v_manufacturer_id
        AND normalized_part_number = public.normalize_code(v_part_number) LIMIT 1;
      IF v_part_id IS NULL THEN
        INSERT INTO public.parts (manufacturer_id, primary_part_number, description, notes)
        VALUES (v_manufacturer_id, v_part_number, v_part_description,
    ...
    IF v_part_number IS NULL AND v_manufacturer_id IS NOT NULL
      ...
      IF v_catalog_number IS NOT NULL THEN
        ...
        WHERE manufacturer_id = v_manufacturer_id
          ...
          AND COALESCE(revision, '') = COALESCE(v_revision, '') LIMIT 1;
      ELSE
        SELECT id INTO v_catalog_id FROM public.catalogs
        WHERE manufacturer_id = v_manufacturer_id
          AND machine_model_id IS NOT DISTINCT FROM v_model_id
      ...
      IF v_catalog_id IS NULL THEN
        IF v_catalog_title IS NULL THEN RAISE EXCEPTION 'Catalog title is required.'; END IF;
        INSERT INTO public.catalogs
          (manufacturer_id, machine_model_id, catalog_number, title, catalog_type,
           language, revision, serial_from, serial_to, source_id, external_source_reference)
        VALUES
          (v_manufacturer_id, v_model_id, v_catalog_number, v_catalog_title, v_catalog_type,
           v_language, v_revision, v_serial_from, v_serial_to, p_source_id, v_external_reference)
    ...
    VALUES
      (v_job_id, v_external_reference,
       CASE WHEN v_part_number IS NOT NULL THEN 'part' ELSE 'catalog' END,
       'imported', COALESCE(v_part_id, v_catalog_id, v_model_id));
